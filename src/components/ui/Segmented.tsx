...
/**
 * Tab switcher. The active pill is a solid lime chip, matching the way the
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      role="tablist"
      className={cn('flex gap-1 rounded-full border-2 border-line bg-panel p-1', className)}
    >
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              active ? 'bg-lime text-ink' : 'text-muted active:bg-panel-2',
            )}
          >
            {option.label}
            {option.count !== undefined && option.count > 0 && (
              <span
                className={cn(
                  ...
                  active ? 'bg-ink/15 text-ink' : 'bg-panel-2 text-faint',
                )}
              >
                {option.count}
              </span>
